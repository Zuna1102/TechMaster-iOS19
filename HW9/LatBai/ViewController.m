...
-(void)creat
{
    ...
    for (int i=0; i<[randomImages count]; i++) {
        ...
        if (i<[randomImages count]/2) {
            ...
            UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(20*i+30*i, 20, 50, 50)];
            
            imageView.tag=i;
            
            LaBai * square  = [randomImages objectAtIndex:i];
...

{
    ...
    if (open==1&&[self.view viewWithTag:sender.view.tag+[randomImages count]].hidden==NO) {
        ...
        LaBai *i= [randomImages objectAtIndex:tag-[randomImages count]];
        
        LaBai *j = [randomImages objectAtIndex:sender.view.tag];
